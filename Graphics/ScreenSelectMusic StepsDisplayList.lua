	...
	Def.StepsDisplayList{
		...
		CursorP1=LoadActor(THEME:GetPathG('DifficultyList', 'cursor'))..{
			...
			InitCommand=function(self)
				self:xy(-190,4):player(PLAYER_1):diffuse(PlayerColor(PLAYER_1)):bounce():effectmagnitude(-10,0,0):effectclock("beat"):effectperiod(1):effectoffset(0.2)
			end,
		...
		CursorP2=LoadActor(THEME:GetPathG('DifficultyList', 'cursor'))..{
			Name='CursorP2',
			InitCommand=function(self)
				self:xy(80,4):rotationz(180):player(PLAYER_2):diffuse(PlayerColor(PLAYER_2)):bounce():effectmagnitude(10,0,0):effectclock("beat"):effectperiod(1):effectoffset(0.2)
			end,
			ChangeCommand=function(self)
				self:stoptweening():decelerate(0.15)
			end
