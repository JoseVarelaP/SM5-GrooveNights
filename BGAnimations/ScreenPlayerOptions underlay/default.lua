...
t[#t+1] = Def.Quad{
	OnCommand=function(self)
		...
		:stretchto( SCREEN_WIDTH, SCREEN_HEIGHT-30, 0, 30 )
		:diffuse(Color.Black)
	end
}
...
for pn in ivalues( GAMESTATE:GetHumanPlayers() ) do
	...
	local DCS = PDir and LoadModule("Config.Load.lua")("DefaultComboSize",PDir) or GAMESTATE:Env()["DefaultComboSizeMachinetemp"..pn]
		...
		t[#t+1] = Def.ActorFrame{
			OnCommand=function(self)
				self:xy( SCREEN_CENTER_X-160*side(pn)-8, SCREEN_CENTER_Y+150 )
			end,
			...
			Def.Quad {
				...
				OnCommand=function(self) self:zoomto(64,64):xy(-114,-2):croptop(0.7):fadetop(0.1):diffuse(Color.Black) end
			},
			Def.Sprite {
				Texture=THEME:GetPathG("","AvatarFrame");
				OnCommand=function(self)
					self:xy(-114,0):diffuse( color("#1C2C3C") )
				end
			},
			Def.BitmapText {
				...
				OnCommand=function(self)
					self:xy(-114,20):zoom(0.5):diffuse( PlayerColor(pn) )
				end
			},
			...
			Def.BitmapText{
				...
				Font="novamono/36/_novamono 36px",
				OnCommand=function(self)
					self:halign(0):zoom(0.55):maxwidth(70)
					:xy( -60+longest, -26+( 16*(0) ) )
			...
			Def.BitmapText{
				Font="novamono/36/_novamono 36px",
				OnCommand=function(self)
					self:halign(0):zoom(0.55):maxwidth(330 - longest)
					:xy( -60+longest, -26+( 16*(2) ) )
				...
				["OptionRowSteps"..ToEnumShortString(pn).."ChangedMessageCommand"]=function(self,param)
					if param.Index then
		...
		t[#t+1] = Def.Sprite{
			...
			DefaultJudgmentOpacityChangeMessageCommand=function(s,param)
				if param.pn == pn and param.choice then
					s:diffusealpha( (param.choice-1)/10 )
				end
			end
		}
		...
		t[#t+1] = Def.BitmapText{
			...
			DefaultComboSizeChangeMessageCommand=function(self,param)
				if param.pn == pn and param.choice then
					self:finishtweening():zoom( 0.75*(param.choice)/10 ) curzoom = (param.choice-1)/10
				end
			end
		}
end
...
t[#t+1] = Def.Sprite{
	...
	OnCommand=function(self)
		self:diffusealpha(0.15):rotationy(180):blend("BlendMode_Add"):xy(SCREEN_CENTER_X,SCREEN_CENTER_Y):zoom(1.1*(SCREEN_WIDTH/640))
	end
}
